...
// this tries to print scens in the macros directory, so use relative path
  ...
  private void execute0() {   
    ...
    for (Scene scene : simulation_0.getSceneManager().getScenes()) {
        ...
        // scene.printAndWait(resolvePath("scenes/" + scene.getPresentationName() + ".jpg"), 1, 800, 600);
        scene.printAndWait(resolvePath("../cases/" + caseName.getText() + "/scenes/" + scene.getPresentationName() + ".jpg"), 1, 800, 600);
    ...
    for (StarPlot plot : simulation_0.getPlotManager().getObjects()) {
         ...
         plot.encode(resolvePath("../cases/" + caseName.getText() + "/scenes/" + plot.getPresentationName() + ".jpg"), "jpg", 800, 600);
         

    };

    // // now use this bash script to archive the images, and give numbering to filenames
    // String cmd = "archive_images.sh";
