...
%% Parent Domain: Bathymetry and Coastlines
% =============================================================================
...
OPTIONS.z0                  = 0;               	% zero?  to-do: this could also be set by the ROMS data, and it changes each time step


% OPTIONS.aa                  = [-122.69 -122.68 48.152 48.1569];         % ULTRA TINY smaller ~3 million cells test domain (runs fast for debugging) 

OPTIONS.dirTopo             = '1_psdem';    % if Seabed_Source=PSDEM then use fileTopo                    
...
%% Turbines 
% =============================================================================
OPTIONS.turbineFile         = 'rotors.csv';     % name of turbine input file, or can be empty [] if want to place turbines manually with GUI
...
%% Computational Setup
% =============================================================================
OPTIONS.typeDEM     = 'shadem';    % 'shadem' or 'dem', changes the appearance of figures
...
OPTIONS.nCPUs       = 16;          % if runOnHPC=false, number of CPUs to run on local computer, ignored if running on supercomputer (on HPC set CPUs within a PBS/SLURM script)
...
OPTIONS.casename      = 'time-1936-noTurbines';
OPTIONS.turbineFile   = false;
